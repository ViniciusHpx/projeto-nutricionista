...
#Aqui eu so permito acessar a view se a pessoa estiver logada
...
def pacientes(request):
    ...
    if request.method == "GET":
        ...
        if (len(nome.strip()) == 0) or (len(sexo.strip()) == 0) or (len(idade.strip()) == 0) or (len(email.strip()) == 0) or (len(telefone.strip()) == 0):
            ...
            print(nome, sexo, idade, email, telefone)
        ...
        try:
            p1 = Pacientes(
                ...
                email = email,
                telefone = telefone,
                #aqui eu estou referenciando quem stá logado cadastrando o paciente
                nutri = request.user
            )
            p1.save()
...
def dados_paciente(request, id ):
    ...
    if request.method == "GET":
        ...
        if (not peso.isnumeric()) or (not altura.isnumeric()) or (not gordura.isnumeric()) or (not musculo.isnumeric()) or (not hdl.isnumeric()) or (not ldl.isnumeric()) or (not colesterol_total.isnumeric()) or (not triglicerídios.isnumeric()):
            messages.add_message(request, constants.ERROR, 'Digite valores númericos')
            return redirect(f'/dados_pacientes/{id}')
        
        elif (len(peso.strip()) == 0) or (len(altura.strip()) == 0) or (len(gordura.strip()) == 0) or (len(musculo.strip()) == 0) or (len(hdl.strip()) == 0) or (len(ldl.strip()) == 0) or (len(colesterol_total.strip()) == 0) or (len(triglicerídios.strip()) == 0):
...
def grafico_peso(request, id):
    ...
    # Aqui eu to setando dentro de uma lista todas as pesagens do paciente 
