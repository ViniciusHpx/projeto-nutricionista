...
def nama_is_valid(request, nome):
    if len(nome) < 3:
        messages.add_message(request, constants.ERROR,'Seu nome está invalido')
        return False
...
def email_is_valid(request, email):
    if not re.match(r"[^@]+@[^@]+\.[^@]+", email):
        messages.add_message(request, constants.ERROR,'E-mail está invalido')
        return False
    return True
